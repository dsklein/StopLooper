...
// CMS3 and CORE
#include "CMS3.h"
#include "../StopAnalysis_80x/CORE/Tools/dorky/dorky.h"
#include "../StopAnalysis_80x/CORE/Tools/goodrun.h"
...
int looperCR0b( analysis* myAnalysis, sample* mySample, int nEvents = -1, bool fast = true) {
	...
	while ( (currentFile = (TFile*)fileIter.Next()) ) {
		...
		for( unsigned int event = 0; event < nEventsTree; ++event) {
			...
			if( Mlb_closestb() > 0. ) myMlb = context::Mlb_closestb();
			else myMlb = context::Mlb_lead_bdiscr();
